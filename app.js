...
authReq.onupgradeneeded = function (e) {
  authDB = e.target.result;
  authDB.createObjectStore("users", { keyPath: "username" });
};

authReq.onsuccess = function (e) {
  authDB = e.target.result;
  createDefaultUser();
};

authReq.onerror = () => console.error("AuthDB gagal dibuka");
...
async function generateWord() {
  ...
  try {
    ...
    const htmlContent =
      ...
      waktuPelaksanaan +
      " (Tiga Puluh) hari Kalender terhitung di tanda tangani Surat Perintah Kerja (SPK)</p>" +
      "<br><br>" +
